...
class SdkAgent {
  constructor(instance, _) {
    this.Sdk = instance;
    this._ = _;
    /** onlaunch触发标志，sdk内部使用，重写 App和Page类，如果是 百度小程序，需要做额外处理 */
...
class Sdk {
  ...
  getSystem() {
    ...
    _.info.callback = function (systemInfo) {
      ...
      console.log('获取系统信息并保存下来：', systemInfo)
  ...
  send(data, callback) {
    ...
    if (this.getConfig('sendConfig').start) {
      if (typeof this.getConfig('sendConfig').fn === 'function') {
        ...
        this.getConfig('sendConfig').fn({
          url: url,
          method: 'GET',
          success: successFn,
          fail: failFn,
          data: _.HTTPBuildQuery(data)
        });
      }
    } else {
      sendData();
